...
from config import DATA_DIR, STRUCT_ARTICLE_RE, STRUCT_CLAUSE_RE, STRUCT_POINT_RE
...
def _article_spans(text: str):
    matches = list(STRUCT_ARTICLE_RE.finditer(text))
...
def _clause_spans(article_text: str):
    matches = list(STRUCT_CLAUSE_RE.finditer(article_text))
    for idx, match in enumerate(matches):
...
def _point_spans(clause_text: str):
    matches = list(STRUCT_POINT_RE.finditer(clause_text))

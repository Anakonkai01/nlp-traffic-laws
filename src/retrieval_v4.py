...
class RetrievalV4Config:
    ...
    use_ce: bool = field(default_factory=lambda: _env_bool("RAG_V4_USE_CE", True))
...
def build_retriever(vs, cfg: RetrievalV4Config | None = None) -> Callable[[str, int], list[Document]]:
    ...
    def retrieve(question: str, top_k: int) -> list[Document]:
        ...
        if cfg.use_ce:
            ce_query = search_query if cfg.use_query_expansion else question
            fused = _ce_rerank(ce_query, fused, cfg.ce_k, cfg.ce_model)
